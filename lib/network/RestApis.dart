...
Future updateProfile(
  ...
  multiPartRequest.fields['username'] =
      userName ?? shared_pref.getString(USER_NAME).validate();
  multiPartRequest.fields['email'] =
      userEmail ?? shared_pref.getString(USER_EMAIL).validate();
  multiPartRequest.fields['name'] =
      name ?? shared_pref.getString(NAME).validate();
  multiPartRequest.fields['contact_number'] =
      contactNumber ?? shared_pref.getString(USER_CONTACT_NUMBER).validate();
  multiPartRequest.fields['address'] =
      address ?? shared_pref.getString(USER_CONTACT_NUMBER).validate();

  if (image != null) {
    print('Call');
    multiPartRequest.files.add(await MultipartFile.fromBytes(
        'profile_image', image,
        filename: fileName));
  }
...
Future<OrderListModel> getAllOrder(
    {int? page, int perPage = 10, String? status}) async {
  return OrderListModel.fromJson(await handleResponse(await buildHttpResponse(
      status != null
          ? 'order-list?page=$page&status=$status&per_page=$perPage'
          : 'order-list?page=$page&status=trashed&per_page=$perPage',
      method: HttpMethod.GET)));
}
